	...
	<view>
		<view class="example-body">
			<uni-card
				...
				subTitle="李俊林"
				mode="title"
				thumbnail="/static/icon/logo.png"
				extra="店长"
				note="true"
				@click="clickCard"
			>
		...
		<uni-list style="text-align: left;">
			...
			<navigator url="room/roomRoomType">
			...
			<navigator url="company/company">
				<uni-list-item v-if="platform" title="协议公司" :show-extra-icon="true" :extra-icon="{ color: '#999999', size: '22', type: 'chat-filled' }" />
			</navigator>
			<navigator url="team/teamIndex">
				<uni-list-item v-if="platform" title="团队管理" :show-extra-icon="true" :extra-icon="{ color: '#343a40', size: '22', type: 'chatboxes-filled' }" />
			</navigator>
		...
		<button class="logout" @click="moreLogin" type="default">多酒店登录</button>
		<button class="logout" @click="logout" type="warn">退出登录</button>
		<button class="logout" plain="true" @click="logoutAll" type="warn">退出全部登录</button>
	</view>
</template>
...
export default {
	data() {
		return {
			platform: false //是否有协议公司权限
		};
	},
	onShow() {
		uni.getStorageSync('authList').indexOf('|208|') > -1 ? (this.platform = true) : (this.platform = false);
	},
	methods: {
		logout() {
			let that = this;
